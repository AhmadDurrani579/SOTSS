...
class SEEventEvaluationVC: UIViewController , NVActivityIndicatorViewable {
    ...
    func getTheEvaluationOfUser(){
        ...
        WebServiceManager.post(params:loginParam as Dictionary<String, AnyObject> , serviceName: EVENTEVALUATION, isLoaderShow: false, serviceType: "Login", modelType: Session.self, success: { (response) in
            self.stopAnimating()
    ...
    func submitEvaluation(){
        ...
        let idOfUsers = UserDefaults.standard.string(forKey: "id")
        let eventId = eventObj?.event_id!
        let evaluationId  = self.responseObj?.result?.eventEvaluation?.evaluationObject?.evaluation_id
        let loginParam =  [ "evaluation_id"         : evaluationId! ,
                             "user_id"          : idOfUsers!//"42620"
...
extension SEEventEvaluationVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView?   {
        ...
        headerCell.lblQuestion.text = responseObj?.result?.eventEvaluation?.sessionQuestionObject?.sessionQuestion![section].question_desc
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        ...
        return responseObj?.result?.eventEvaluation?.sessionQuestionObject?.sessionQuestion?.count ?? 0
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return (responseObj?.result?.eventEvaluation?.sessionQuestionObject?.sessionQuestion![section].option?.count)!
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.lblOption.text = responseObj?.result?.eventEvaluation?.sessionQuestionObject?.sessionQuestion![indexPath.section].option![indexPath.row].option_desc
        return cell!
